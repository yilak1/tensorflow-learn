...
# match_filenames_once()保存匹配模式的文件列表，因此只计算一次,返回的文件的顺序可能是不确定的
# 返回：
# 一个变量，它被初始化为与模式匹配的文件列表。
test_files = tf.train.match_filenames_once("output_test.tfrecords")
print("match_filenames_once testfile", test_files)

# 2.定义解析TFRecord文件的parse方法
def parser(record):
    ...
    decoded_raw = tf.decode_raw(features['image_raw'], tf.uint8)
    retyped_images = tf.cast(decoded_raw, tf.float32)
    images = tf.reshape(retyped_images, [784])
    labels = tf.cast(features['label'], tf.int32)
    return images, labels

# 3.定义训练数据集
image_size = 299         # 定义神经网路输入层图片的大小
...
# 4.定义神经网络结构和优化过程
# tf.multiply 是点乘，matmul是叉乘
def inference(input_tensor, weights, biases1, weights2, biases2):
...
# 计算交叉熵及其平均值,当分类只有一个正确答案时可以用sparse_softmax...作为交叉熵函数
cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=y, labels=label_batch)
# cross_entropy shape=(batch_size)
cross_entropy_mean = tf.reduce_mean(cross_entropy)
# corss_entropy_mean shape=(1)
